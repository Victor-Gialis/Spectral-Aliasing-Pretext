"""
Script simple d'optimisation des hyperparamètres du SAP avec Optuna.
...
def objective(trial: Trial) -> float:
    ...
    Suggère des hyperparamètres et lance l'entraînement du SAP.
    Retourne la validation loss comme métrique à minimiser.
    """
    
    # Suggérer des hyperparamètres
    batch_size = trial.suggest_categorical('batch_size', [32, 64, 128, 256, 512])
    downsampling_factor = trial.suggest_categorical('downsampling_factor',['None', 2, 4, 8])
    ...
    print(f"downsampling_factor: {downsampling_factor}")
...
def extract_validation_loss() -> float:
    ...
    try:
        ...
        # Chercher dans tous les répertoires SAP/CWRU
        sap_dirs = list((pretrain_dir / "SAPModel" / (CONFIG['pretrain_dataset']+'Dataset')).glob("*/log/metrics.csv"))
        
        if not sap_dirs:
            return None
        
        # Prendre le plus récent
        latest_metrics = max(sap_dirs, key=lambda p: p.stat().st_mtime)
...
def run_optimization():
    ...
    print(f"\n{'='*60}")
    print(f"OPTIMISATION OPTUNA - SAP")
    print(f"{'='*60}\n")
...
def save_results(study: optuna.Study):
    ...
    output_dir = Path("results/sap_optimization")
