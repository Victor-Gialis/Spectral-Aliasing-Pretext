"""
Script d'optimisation des hyperparamètres du TS2Vec avec Optuna.
...
def objective(trial: Trial) -> float:
    ...
    Suggère des hyperparamètres et lance l'entraînement du TS2Vec.
    ...
    temporal_unit = trial.suggest_int('temporal_unit', 0, 3)
    projection_dim = trial.suggest_categorical('projection_dim', [32, 64, 128, 256])
    epochs = 30
...
def extract_validation_loss() -> float:
    ...
    try:
        ...
        # Chercher dans tous les répertoires TS2Vec/CWRU
        ts2vec_dirs = list((pretrain_dir / "TS2VecModel" / (CONFIG['pretrain_dataset']+'Dataset')).glob("*/log/metrics.csv"))
        
        if not ts2vec_dirs:
            return None
        
        # Prendre le plus récent
        latest_metrics = max(ts2vec_dirs, key=lambda p: p.stat().st_mtime)
...
def run_optimization():
    """Lance l'optimisation Optuna et affiche les résultats."""
    
    print(f"\n{'='*60}")
    print(f"OPTIMISATION OPTUNA - TS2VEC")
    print(f"{'='*60}\n")
...
def save_results(study: optuna.Study):
    ...
    output_dir = Path("results/ts2vec_optimization")
